"""Exact-version, provenance-preserving resume-to-job alignment reads."""
...
def account():
    ...
    response = client.post(
        ...
        json={"email": f"alignment-{uuid4().hex}@example.com", "password": "Alignment-password-123!"},
    )
...
def catalog(connection):
    skills = {name: uuid4() for name in ("Python", "Docker", "AWS", "Terraform")}
    connection.execute(
        ...
        [
            {
                "id": skill_id,
                "name": name,
                "slug": f"alignment-{name.casefold()}",
                "category": "TECHNOLOGY",
            }
...
def job_requirements(connection, job_id, skills):
    connection.execute(
        ...
        [
            ...
            for name in ("Python", "Docker", "AWS", "Terraform")
        ],
    )
...
def add_project_technology(connection, owner, skill_id):
    ...
    connection.execute(
        Project.__table__.insert(),
        {
            ...
            "description": "Free text is not used for alignment.",
        },
    )
    connection.execute(
...
def test_alignment_classifies_exact_selected_version_and_preserves_provenance(isolated_database):
    client, owner = account()
    skills = catalog(isolated_database)
    job_id = seed_job(
        isolated_database, title="Alignment job", external_id="alignment-job", posted_at=None
    )
    ...
    response = client.get(
        f"/api/v1/jobs/{job_id}/resume-alignment", params={"resume_version_id": selected_id}
    )
    ...
    assert {
        key: body[key]
        for key in ("job_id", "job_title", "company_name", "resume_id", "resume_title", "resume_version_id", "resume_version_number")
    } == {
        "job_id": str(job_id),
        "job_title": "Alignment job",
        "company_name": "Alignment job Company",
        "resume_id": str(resume_id),
        "resume_title": "Targeted resume",
        "resume_version_id": str(selected_id),
        "resume_version_number": 1,
    }
    assert [item["name"] for item in body["shown_on_resume"]] == ["Python"]
    assert [item["name"] for item in body["candidate_evidence_not_shown"]] == ["AWS", "Docker"]
    assert [item["name"] for item in body["no_recorded_evidence"]] == ["Terraform"]
    assert {item["requirement_id"] for item in body["unknown_unassessed"]} == {
        str(education_id),
        str(eligibility_id),
    }

    shown_source = body["shown_on_resume"][0]["sources"]
    assert shown_source[0]["source_type"] == "RESUME_EVIDENCE"
    assert shown_source[0]["source_id"] == str(selected_item)
    assert shown_source[0]["resume_version_id"] == str(selected_id)
    candidate = {item["name"]: item for item in body["candidate_evidence_not_shown"]}
    assert candidate["Docker"]["sources"][0]["source_type"] == "CONFIRMED_BY_USER"
    assert candidate["Docker"]["sources"][0]["source_id"] == str(confirmed_id)
    assert candidate["AWS"]["sources"][0]["source_type"] == "PROJECT"
    assert candidate["AWS"]["sources"][0]["source_id"] == str(project_id)
    assert candidate["AWS"]["sources"][0]["project_title"] == "Catalog technology project"
    # Terraform is linked only to another version and cannot support selected version A.
    assert body["no_recorded_evidence"][0]["sources"] == []
...
def test_alignment_is_owner_scoped_read_only_and_handles_missing_records_safely(isolated_database):
    client, owner = account()
    other, _ = account()
    skills = catalog(isolated_database)
    job_id = seed_job(
        isolated_database, title="Private alignment", external_id="private-alignment", posted_at=None
    )
    ...
    missing_version = client.get(
        f"/api/v1/jobs/{job_id}/resume-alignment", params={"resume_version_id": uuid4()}
    )
    missing_job = client.get(
        f"/api/v1/jobs/{uuid4()}/resume-alignment", params={"resume_version_id": selected_id}
    )
    unauthenticated = TestClient(app).get(
        f"/api/v1/jobs/{job_id}/resume-alignment", params={"resume_version_id": selected_id}
    )
    assert [response.status_code for response in (inaccessible, missing_version, missing_job, unauthenticated)] == [
        404,
        404,
        404,
        401,
    ...
    assert all(
        ...
        for response in (inaccessible, missing_version, missing_job, unauthenticated)
    )


def test_alignment_rejects_unsafe_or_ambiguous_query_inputs(isolated_database):
    client, owner = account()
    skills = catalog(isolated_database)
    job_id = seed_job(
        isolated_database, title="Query alignment", external_id="query-alignment", posted_at=None
    )
    job_requirements(isolated_database, job_id, skills)
    _, selected_id, _, _, _ = resume_versions(
        isolated_database, owner, skills["Python"], skills["Terraform"]
    ...
    cases = (
        client.get(f"/api/v1/jobs/{job_id}/resume-alignment"),
        client.get(
            f"/api/v1/jobs/{job_id}/resume-alignment",
...
def test_alignment_never_infers_from_raw_text_preferences_or_free_text(isolated_database):
    client, owner = account()
        ...
        [
            {
                "id": required_id,
                "name": "Python",
                "slug": "alignment-required-python",
                "category": "TECHNOLOGY",
            },
            {
                "id": same_named_id,
                "name": "Python",
                "slug": "alignment-other-python",
                "category": "TECHNOLOGY",
            },
        ],
    )
    job_id = seed_job(
        ...
        external_id="alignment-inference-boundary",
        posted_at=None,
    )
    isolated_database.execute(
        JobSkillRequirement.__table__.insert(),
    ...
    response = client.get(
        f"/api/v1/jobs/{job_id}/resume-alignment", params={"resume_version_id": version_id}
    )
    assert response.status_code == 200, response.text
    assert response.json()["shown_on_resume"] == []
    assert response.json()["candidate_evidence_not_shown"] == []
    assert [item["requirement_id"] for item in response.json()["no_recorded_evidence"]] == [
        str(required_id)
    ]
